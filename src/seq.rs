...
pub trait SliceRandom {
    ...
    fn pick_multiple<'a, R: Rng>(&'a self, rng: &'a mut R, amount: usize)
...
impl<T> SliceRandom for [T] {
    ...
    fn pick_multiple<'a, R: Rng>(&'a self, rng: &'a mut R, amount: usize)
        -> RandomSampler<'a, Self::Item, R>
    {
        // FIXME: what to do if `amount == 0`, or `amount >= self.len()`?
...
impl<'a, T: 'a, R: Rng + 'a> Iterator for RandomSampler<'a, T, R> {
    ...
    fn next(&mut self) -> Option<&'a T> {
        if self.sampler.n > 1 {
            self.index += self.sampler.calculate_skip(self.rng);
...
D1. [Is n ≥ αN?] If n ≥ αN, use Algorithm A to do the
    sampling and then terminate the algorithm.
    (Otherwise, we use the rejection technique of
    Steps D2-D5.)
D2. [Generate U and X.] Generate a random variate U
    that is uniformly distributed between 0 and 1
    and a random variate X that has density function
    or probability function g(x).
D3. [Accept?] If U ≤ h(⌊X⌋)/cg(X), then set S := ⌊X⌋
    and go to Step D5.
D4. [Accept?] If U ≤ f(⌊X⌋)/cg(X), then set S := ⌊X⌋.
    Otherwise, return to Step D2.
D5. [Select the (S + 1)st record.] Skip over the next
    S(n, N) records in the file and select the follow-
    ing one for the sample. Set N := N - S(n, N) - 1
    and n := n - 1. Return to Step D2 if n > 0. ∎
*/
...
// Typical values of α can be expected in the range 0.05-0.15. We follow the
// suggestion in the paper of 1/13.
const ALPHA_INV: usize = 13; // (1.0 / α)
// 13 means that at least 1 in 13 of the remaining elements should be sampled.
...
impl SequentialRandomSampler {
    fn new<R: Rng>(n: usize, total: usize, rng: &mut R) -> Self {
        Self {
            remaining: total,
            n: n,
            v_prime: rng.gen::<f64>().powf(1.0 / (n as f64)),
    ...
    fn method_a_skip<R: Rng>(&mut self, rng: &mut R) -> usize {
        ...
        self.remaining = remaining_f as usize;
        skip
    }
    ...
    fn method_d_skip<R: Rng>(&mut self, rng: &mut R) -> usize {
        ...
        loop {
            ...
            let mut x;
            loop {
                x = n_f * (1.0 - self.v_prime);
                skip = x as usize;
                if skip < qu1 { break; }
                self.v_prime = rng.gen::<f64>().powf(ninv);
            }
            ...
            //
            // Note: `qu1_f == N - n + 1`
            let y1 = (remaining_f * u / qu1_f).powf(nmin1inv);
            self.v_prime = y1 * (qu1_f - qu1_f / skip_f)
                           * (1.0 - x / remaining_f);
            ...
            if remaining_f / (remaining_f - x)
                >= y1 * y2.powf(nmin1inv)
            {
                self.v_prime = rng.gen::<f64>().powf(nmin1inv);
                break; // Accept!
            }
